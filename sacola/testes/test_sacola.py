...
# Teste que verifica o processo de craição do carrinho
...
# Verifica o se possui cqarrinho vazio caso contrario ele somente irá validar o carrinho atual se mudar da pau
...
# Add produto dentro do carrinho vazio
...
# Adiciona um item ao carrinho com uma quantidade > 1
...
# Verifica se é possivel adicionar 2 produtos ao carrinho retornando o resultado da quantidade garantindo que a sessão foi salva
...
# Adiciona o produto ao carrinho vazio e substitui a quantidade do item selecionado não somando apenas subtituindo
...
# Adiciona ao carrinho e remove o produto garantido a modificação e o save do carrinho
...
# Tenta remover o produto que não está no carriho retornado o carrinho vazio garantindo que a session n foi modificada
def test_remove_product_not_in_cart(product, cart, session):
    cart.remove(product)
    assert session[settings.CART_SESSION_ID] == {}
    assert not session.modified

# Cria e adiciona ao carrinho as variedades dos produtos
